...
async function createGetRequest(
  ...
  console.log('🔍 Making request to:', `${baseUrl}${urlPath}`)
  const req = https
    .request({
      path: `${baseUrl}${urlPath}`,
      method: 'GET',
...
export class SocketSdk {
  ...
  async batchPackageFetch(
    ...
    const req = https
      .request({
        path: `${this.#baseUrl}purl?${new URLSearchParams(queryParams)}`,
        method: 'POST',
        ...this.#reqOptions
      })
  ...
  async getOrgFullScan(
    ...
    try {
      const req = https.request({
        path: `${this.#baseUrl}orgs/${encodeURIComponent(orgSlug)}/full-scans/${encodeURIComponent(fullScanId)}`,
        method: 'GET',
        ...this.#reqOptions
      })
      const { 0: res } = await events.once(req, 'response')
